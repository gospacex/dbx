...
func TestName_ReturnsCorrectString(t *testing.T) {
	if got := New().Name(); got != "dbx.tidb" {
		t.Errorf("Name() = %q, want dbx.tidb", got)
	}
}
...
func TestBuild_Success(t *testing.T) {
	// MySQL/TiDB DSN format: user:pass@tcp(host:port)/dbname. A valid local
	// default lets sql.Open succeed without a network round-trip
	// (sql.Open is lazy — it only validates the DSN, doesn't dial).
	cli, err := New().Build("inst", map[string]any{"config": map[string]any{"dsn": "root:pass@tcp(127.0.0.1:4000)/test"}})
	if err != nil {
		// Be tolerant: any build error in CI without TiDB is acceptable
		// (ErrBuildFailed wraps a DSN validation or driver-init error).
		if errors.Is(err, hubx.ErrBuildFailed) {
			t.Skipf("TiDB build skipped: %v", err)
		}
